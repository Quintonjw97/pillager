...
# %%
def crit_search(kwarg=['min','max'],kwarg_search='search'):
    tar = 1.0
    R = 3
    errK = 0.0005
    chgTheta = 1
    ...
    thetas = [0,180]
    write_serpent(thetas[0],100000,kwarg=kwarg[0])
    run_serpent()
    k_min, sdev = get_eigenvalue('serpent.i_res.m')
    fun = [float(k_min)-tar]
    sigmas = [float(sdev)**2]
    
    write_serpent(thetas[1],100000,kwarg=kwarg[1])
    run_serpent()
    k_max, sdev = get_eigenvalue('serpent.i_res.m')
    ...
    while (abs(fun[-1]) > errK or abs(thetas[-1]-thetas[-2]) > chgTheta) and iteration < 6:
        ...
        GRsecant(thetas,sigmas,fun,R)
        if iteration < 4:
            write_serpent(thetas[-1],10000,kwarg=kwarg_search)
        else:
            write_serpent(thetas[-1],100000,kwarg=kwarg_search)
        run_serpent()
        keff, sdev = get_eigenvalue('serpent.i_res.m')
...
def GRsecant(thetas,sigmas,f,R):
    if R >= len(f):
        theta_new = (np.sum(np.divide(thetas,sigmas))*np.sum(np.divide(np.multiply(f,thetas),sigmas)) - np.sum(np.divide(np.square(thetas),sigmas))*np.sum(np.divide(f,sigmas))) / (np.sum(np.divide(np.multiply(f,thetas),sigmas))*np.sum(np.reciprocal(sigmas)) - np.sum(np.divide(f,sigmas))*np.sum(np.divide(thetas,sigmas)))
    else:
       theta_new = (np.sum(np.divide(thetas[-R:],sigmas[-R:]))*np.sum(np.divide(np.multiply(f[-R:],thetas[-R:]),sigmas[-R:])) - np.sum(np.divide(np.square(thetas[-R:]),sigmas[-R:]))*np.sum(np.divide(f[-R:],sigmas[-R:]))) / (np.sum(np.divide(np.multiply(f[-R:],thetas[-R:]),sigmas[-R:]))*np.sum(np.reciprocal(sigmas[-R:])) - np.sum(np.divide(f[-R:],sigmas[-R:]))*np.sum(np.divide(thetas[-R:],sigmas[-R:]))) 
    return thetas.append(theta_new)

def get_eigenvalue(filePath):
    file = open(filePath)
    data = file.readlines()
    file.close()
    for line in data:
...
def write_serpent(angle,particles,kwarg,day=0):
