...
def test_blacklist_dir_files(obj=t_class):
    ...
    assert set(obj.file_blacklist).issubset([obj.input_base,obj.control_base,'serpent_control.i','serpent.i.wrk','outputs.csv','serpent.i_det0.m','serpent.i_res.m'])
    assert len(obj.file_blacklist) - 2 == len([obj.input_base,obj.control_base,'serpent_control.i','serpent.i.wrk','outputs.csv','serpent.i_det0.m','serpent.i_res.m'])
'''
...
def test_write_serpent(obj=t_class):
    obj.write_serpent(1000,1e9,'initial')
    with open(obj.output_dir + 'serpent_control.i','r', encoding='utf-8') as f:
        contents1 = f.readlines()
    with open(obj.output_dir + 'serpent.i','r', encoding='utf-8') as f:
        contents2 = f.readlines()
    os.remove(obj.output_dir + 'serpent.i')
    assert set(contents1).issubset(contents2)
